...
class UltraEditDataset(Dataset):
    def __init__(self, split='train', split_ratio=0.8, random_seed=42):
        from datasets import load_dataset
        import random
        
        # Load the full dataset
        self.dataset = load_dataset("BleachNick/UltraEdit_Region_Based_100k")['RegionBase']
    ...
    def __getitem__(self, idx):
        ...
        # Extract the editing prompt (edit_prompt)
        prompt = data_item['edit_prompt']
        
        # Create metadata that includes edit_object for compatibility
        metadata = {
            'edit_object': data_item['edit_object'],
            'source_image': data_item['source_image'],
            'edited_image': data_item['edited_image'],
            'mask_image': data_item['mask_image']
        }
        
        # Get source image as reference image
        source_image = data_item['source_image']
        ...
        item = {
            "prompt": prompt,
            "metadata": metadata,
            "image": source_image,  # Use source image as input
            "ref_image": source_image,  # Reference image for metrics
            "mask_image": mask_image,  # Mask for region-based editing
            "prompt_with_image_path": f"{prompt}_{actual_idx}"
        }
    ...
    def collate_fn(examples):
        prompts = [example["prompt"] for example in examples]
        metadatas = [example["metadata"] for example in examples]
        images = [example["image"] for example in examples]  # Source images
        ref_images = [example["ref_image"] for example in examples]  # Reference images for metrics
        mask_images = [example["mask_image"] for example in examples]  # Mask images
        prompt_with_image_paths = [example["prompt_with_image_path"] for example in examples]
        return prompts, metadatas, images, prompt_with_image_paths, ref_images, mask_images
...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        prompts, prompt_metadata, ref_images, _, mask_images = test_batch[:5]
        ...
        rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, ref_images, mask_images, only_strict=False)
...
def main(_):
    ...
    # Use UltraEditDataset for the new HuggingFace dataset
    train_dataset = UltraEditDataset(split='train', split_ratio=0.8, random_seed=42)
    test_dataset = UltraEditDataset(split='test', split_ratio=0.8, random_seed=42)
    ...
    train_dataloader = DataLoader(
        ...
        num_workers=0,
        collate_fn=UltraEditDataset.collate_fn,
    ...
    while True:
        ...
        for i in tqdm(
            ...
            prompts, prompt_metadata, ref_images, prompt_with_image_paths, _, mask_images = next(train_iter)
            ...
            rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, ref_images, mask_images, only_strict=True)
            # yield to to make sure reward computation starts
            time.sleep(0)
            samples.append(
                {
